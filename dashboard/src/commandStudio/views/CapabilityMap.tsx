...
const CAPABILITIES = [
  ...
  { group: 'Platform & Deployment', items: [
    ...
    { name: 'AMD Historical Evidence', endpoint: 'GET /api/product/amd/evidence', status: 'wired', safe: true },
    { name: 'AMD Capability Planes', endpoint: 'GET /api/product/amd/capability', status: 'wired', safe: true },
...
export function CapabilityMap() {
  ...
  const [currentRequest, setCurrentRequest] = useState<CurrentRequestPlane | null>(null);
  ...
  const testAmdPath = async () => {
    ...
    setCurrentRequest(pendingCurrentRequest());
    try {
  ...
  const copyReviewerNotes = () => {
    const text = buildReviewerNotes(capability, currentRequest);
    navigator.clipboard.writeText(text).then(
      () => showToast('Reviewer notes copied with historical/live/request separation.', 'success'),
  ...
  const liveRuntime = capability?.live_runtime || null;
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 max-w-[1600px] mx-auto h-full overflow-y-auto">
      <div className="mb-6">
        ...
        <p className="text-slate-500 mt-2">API surface wiring, historical AMD evidence, current configuration, and per-request verification.</p>
      </div>
      ...
      <AmdEvidenceSummary
        ...
        onRefresh={() => void refreshRuntimeStatus(true)}
      />
      ...
      <div className="bg-slate-900 rounded-xl p-5 md:p-6 text-white mb-8 flex flex-col md:flex-row justify-between items-start gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            ...
            <div className="p-4 bg-slate-800 rounded text-sm text-slate-300 font-mono leading-relaxed" data-testid="capability-current-ai-runtime">
              <strong className="text-white block mb-2 font-sans">Current AI Runtime:</strong>
              Configuration: {liveRuntime?.configured ? 'Configured — not live verified by status check' : 'Not configured in this process'}<br/>
              Provider: {liveRuntime?.configured ? liveRuntime.provider || 'Reported by server' : 'Not claimed'}<br/>
              Accelerator: {liveRuntime?.configured ? liveRuntime.accelerator || 'Reported by server' : 'Not claimed'}<br/>
              Runtime: {liveRuntime?.configured ? liveRuntime.runtime || 'Reported by server' : 'Not claimed'}<br/>
              Served model: {liveRuntime?.configured ? liveRuntime.served_model || 'Not reported' : 'Not claimed'}<br/>
              Human review: Required<br/>
              Fallback available: {liveRuntime?.fallback_available === false ? 'No' : 'Yes'}
            </div>
          </div>

          {amdTestResult && (
            <div className={`mt-4 p-4 rounded border text-sm font-mono ${amdVerified ? 'bg-emerald-900/40 border-emerald-500' : amdTestResult.fallback_used ? 'bg-amber-900/30 border-amber-500' : 'bg-red-900/30 border-red-500'}`}>
              <div className={`font-black font-sans text-base mb-2 flex items-center gap-2 ${amdVerified ? 'text-emerald-300' : amdTestResult.fallback_used ? 'text-amber-300' : 'text-red-300'}`}>
                {amdVerified
                  ? '✓ AMD/vLLM PATH VERIFIED LIVE FOR THIS REQUEST'
                  : amdTestResult.fallback_used
                    ? 'FALLBACK USED — NOT VERIFIED LIVE'
              ...
              <div className="grid grid-cols-1 gap-0.5 text-slate-300 text-xs">
                ...
                <div>Fallback used: <span className={amdTestResult.fallback_used ? 'text-amber-300' : 'text-emerald-300'}>{amdTestResult.fallback_used ? 'Yes' : 'No'}</span></div>
              </div>
        ...
        <div className="flex flex-col gap-2 w-full md:w-52 shrink-0">
          ...
          <button
            ...
            data-action-id="amd.test_live_path"
      ...
      <div className="mt-8 p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-3">
        ...
        <p className="text-sm text-emerald-800">
          <strong>Safety boundary intact:</strong> Historical evidence is frozen and labelled, current runtime status is read-only, live status is established per request, and all advisory output remains human-review-required.
        </p>
      </div>
    </div>
  );
