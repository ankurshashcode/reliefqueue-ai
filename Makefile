...
help:
	...
	@echo "  make amd-evidence-validate"
	@echo "  make amd-evidence-ui-check"
	@echo "  make operator"
...
amd-evidence-ui-check:
	...
	$(NPM) --prefix dashboard run amd-evidence-ui-check

submission-pack:
	PYTHONPATH=src $(PYTHON) -m reliefqueue.submission_pack

submission-final-gate:
	PYTHONPATH=src $(PYTHON) scripts/submission_final_gate.py

submission-public-check:
	@if [ -z "$${RELIEFQUEUE_PUBLIC_URL:-}" ]; then echo "RELIEFQUEUE_PUBLIC_URL is required"; exit 2; fi
	PYTHONPATH=src $(PYTHON) scripts/submission_public_check.py "$${RELIEFQUEUE_PUBLIC_URL}"

submission-live-amd-check:
	@if [ -z "$${RELIEFQUEUE_PUBLIC_URL:-}" ]; then echo "RELIEFQUEUE_PUBLIC_URL is required"; exit 2; fi
	@if [ "$${RELIEFQUEUE_CONFIRM_LIVE_AMD:-}" != "YES" ]; then echo "RELIEFQUEUE_CONFIRM_LIVE_AMD=YES is required"; exit 2; fi
	PYTHONPATH=src $(PYTHON) scripts/submission_live_amd_check.py "$${RELIEFQUEUE_PUBLIC_URL}"

amd-quality-offline-validation:
	PYTHONPATH=src $(PYTHON) scripts/amd_quality_live_validation.py

amd-quality-live-validation:
	@if [ "$${RELIEFQUEUE_CONFIRM_LIVE_AMD:-}" != "YES" ]; then echo "RELIEFQUEUE_CONFIRM_LIVE_AMD=YES is required"; exit 2; fi
	AMD_QUALITY_LIVE=1 PYTHONPATH=src $(PYTHON) scripts/amd_quality_live_validation.py
...
replit-run:
	@if [ ! -f dashboard/dist/index.html ]; then \
		echo "dashboard/dist missing; running make replit-build first"; \
		$(MAKE) replit-build; \
	fi
	exec env PYTHONPATH=src $(PYTHON) -m reliefqueue.product_api serve --host 0.0.0.0 --port "$${PORT:-5000}"

replit-smoke: replit-build
	bash scripts/replit_smoke.sh

replit-navigation-smoke: replit-build
	npm --prefix dashboard run replit-navigation-smoke
